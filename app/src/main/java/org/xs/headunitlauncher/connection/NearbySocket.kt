...
class NearbySocket : Socket() {
    ...
    var inputStreamWrapper: InputStream?
        ...
        set(value) {
            ...
            if (value != null) {
                org.xs.headunitlauncher.utils.AppLog.i("NearbySocket: InputStream is now AVAILABLE. Releasing latch.")
    ...
    override fun getInputStream(): InputStream {
        org.xs.headunitlauncher.utils.AppLog.d("NearbySocket: getInputStream() called")
        return object : InputStream() {
            private fun waitForStream(): InputStream {
                if (inputLatch.count > 0L) {
                    org.xs.headunitlauncher.utils.AppLog.i("NearbySocket: Blocking read until InputStream is AVAILABLE via Nearby Payload...")
                }
                inputLatch.await()
    ...
    override fun getOutputStream(): OutputStream {
        org.xs.headunitlauncher.utils.AppLog.d("NearbySocket: getOutputStream() called")
        return object : OutputStream() {
            private fun waitForStream(): OutputStream {
                if (outputLatch.count > 0L) {
                    org.xs.headunitlauncher.utils.AppLog.d("NearbySocket: Waiting for outputLatch...")
                }
                outputLatch.await()
            ...
            override fun write(b: Int) {
                org.xs.headunitlauncher.utils.AppLog.v("NearbySocket: writing 1 byte to pipe")
                waitForStream().write(b)
            }
            
            override fun write(b: ByteArray) = write(b, 0, b.size)
            override fun write(b: ByteArray, off: Int, len: Int) {
                org.xs.headunitlauncher.utils.AppLog.v("NearbySocket: writing $len bytes to pipe")
                waitForStream().write(b, off, len)
                // Force flush since GMS Nearby Stream payloads might buffer a lot
                waitForStream().flush()
            }
            override fun flush() {
                org.xs.headunitlauncher.utils.AppLog.v("NearbySocket: flush() called")

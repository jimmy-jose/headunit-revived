...
import org.xs.hulhelper.BuildConfig
import org.xs.hulhelper.R
import java.io.File
...
object CrashReportStore {
    ...
    fun install(context: Context) {
        ...
        Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
            try {
                ...
                // Best effort only. Do not interfere with the crash handoff.
            }
    ...
    fun sharePendingReport(context: Context): Boolean {
        ...
        context.startActivity(Intent.createChooser(intent, context.getString(R.string.share_crash_report)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
        return true
    }
    ...
    private fun persistCrash(context: Context, thread: Thread, throwable: Throwable) {
        val reportDir = context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS) ?: context.filesDir
        if (!reportDir.exists()) {
            reportDir.mkdirs()
        }
        ...
        val reportFile = File(reportDir, "WirelessHelper_Crash_$fileStamp.txt")
    ...
    private fun rotateReports(reportDir: File) {
        val files = reportDir.listFiles { _, name -> name.startsWith("WirelessHelper_Crash_") }
    ...
    private fun buildReport(
        ...
        return buildString {
            appendLine("HeadUnit Helper crash report")

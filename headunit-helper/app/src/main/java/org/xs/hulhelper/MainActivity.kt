...
class MainActivity : AppCompatActivity() {
    private data class ConnectionModeOption(val mode: Int, val label: String)
    ...
    private lateinit var btnDetectWifiDirectName: Button
    ...
    private var wifiDirectNameDiscoveryManager: WifiDirectNameDiscoveryManager? = null
    private var wifiDirectDetectDialog: androidx.appcompat.app.AlertDialog? = null
    ...
    private val connectionModeOptions by lazy { buildConnectionModeOptions() }
    ...
    private fun initializeViews() {
        ...
        btnDetectWifiDirectName = findViewById(R.id.btnDetectWifiDirectName)
    ...
    private fun setupListeners() {
        ...
        layoutConnectionMode.setOnClickListener {
            val prefs = getSharedPreferences("WirelessHelperPrefs", Context.MODE_PRIVATE)
            val currentMode = normalizeConnectionMode(prefs.getInt("connection_mode", 0))
            val currentSelection = connectionModeOptions.indexOfFirst { it.mode == currentMode }.coerceAtLeast(0)
            MaterialAlertDialogBuilder(this, R.style.DarkAlertDialog)
                .setTitle(R.string.connection_mode_label)
                .setSingleChoiceItems(connectionModeOptions.map { it.label }.toTypedArray(), currentSelection) { dialog, which ->
                    val selectedMode = connectionModeOptions[which].mode
                    prefs.edit { putInt("connection_mode", selectedMode) }
                    tvConnectionModeValue.text = connectionModeOptions[which].label
                    updateModeSpecificUI(selectedMode)
                    
                    if (selectedMode == 1 || selectedMode == 2) { // Phone or Tablet Hotspot
        ...
        btnDetectWifiDirectName.setOnClickListener { ensureWifiDirectDetectPermissionsAndStart() }
    }
    ...
    private fun restoreState() {
        ...
        val connMode = normalizeConnectionMode(prefs.getInt("connection_mode", 0))
        if (connMode != prefs.getInt("connection_mode", 0)) {
            prefs.edit { putInt("connection_mode", connMode) }
        }
        tvConnectionModeValue.text = connectionModeOptions.firstOrNull { it.mode == connMode }?.label
            ?: connectionModeOptions.first().label
    ...
    private fun migrateSettings(prefs: android.content.SharedPreferences) {
        ...
        if (currentDirectSet == null) {
            ...
            val newSet = mutableSetOf("HUL") // Always default to HUL
            if (!oldDirectName.isNullOrEmpty() && oldDirectName != "HUL") {
    ...
    private fun updateModeSpecificUI(mode: Int) {
        ...
        btnDetectWifiDirectName.visibility = if (mode == MODE_WIFI_DIRECT) View.VISIBLE else View.GONE
    ...
    private fun normalizeConnectionMode(mode: Int): Int {
        return if (mode == MODE_WIFI_DIRECT && !deviceSupportsWifiDirect()) MODE_NSD else mode
    }
    ...
    private fun ensureWifiDirectDetectPermissionsAndStart() {
        ...
        startWifiDirectNameDetection()
    }

    private fun startWifiDirectNameDetection() {
        val manager = wifiDirectNameDiscoveryManager ?: WifiDirectNameDiscoveryManager(applicationContext).also {
            wifiDirectNameDiscoveryManager = it
        }

        if (!manager.hasRequiredPermissions()) {
            Toast.makeText(this, R.string.wifi_direct_detect_permissions_required, Toast.LENGTH_LONG).show()
            return
        }

        showWifiDirectDetectDialog()
        manager.start(object : WifiDirectNameDiscoveryManager.Callback {
            override fun onSearching() {
                runOnUiThread { showWifiDirectDetectDialog() }
            }

            override fun onDiscovered(name: String) {
                runOnUiThread {
                    wifiDirectDetectDialog?.dismiss()
                    promptToAddWifiDirectName(name)
                }
            }
    ...
    private fun showWifiDirectDetectDialog() {
        if (wifiDirectDetectDialog?.isShowing == true) return
        val dialogView = LayoutInflater.from(this).inflate(R.layout.dialog_wifi_direct_detect_progress, null)
        wifiDirectDetectDialog = MaterialAlertDialogBuilder(this, R.style.DarkAlertDialog)
            .setTitle(R.string.detect_wifi_direct_name)
            .setView(dialogView)
            .setNegativeButton(android.R.string.cancel) { _, _ ->
                wifiDirectNameDiscoveryManager?.stop()
            }
            .setOnDismissListener {
                wifiDirectDetectDialog = null
            }
            .show()
    }

    private fun promptToAddWifiDirectName(name: String) {
        val prefs = getSharedPreferences("WirelessHelperPrefs", Context.MODE_PRIVATE)
        val names = prefs.getStringSet("wifi_direct_target_names", mutableSetOf("HUL"))?.toMutableSet()
            ?: mutableSetOf("HUL")

        if (names.contains(name)) {
            Toast.makeText(this, getString(R.string.wifi_direct_name_already_added, name), Toast.LENGTH_LONG).show()
            return
        }

        MaterialAlertDialogBuilder(this, R.style.DarkAlertDialog)
            .setTitle(R.string.detect_wifi_direct_name)
            .setMessage(getString(R.string.detect_wifi_direct_name_confirm, name))
            .setPositiveButton(R.string.add) { _, _ ->
                names.add(name)
                prefs.edit { putStringSet("wifi_direct_target_names", names) }
                updateWifiDirectValueDisplay()
                Toast.makeText(this, getString(R.string.wifi_direct_name_added, name), Toast.LENGTH_LONG).show()
            }
            .setNegativeButton(android.R.string.cancel, null)
            .show()
    }
    ...
    private fun showWifiDirectNameSelector() {
        val prefs = getSharedPreferences("WirelessHelperPrefs", Context.MODE_PRIVATE)
        val names = prefs.getStringSet("wifi_direct_target_names", mutableSetOf("HUL"))?.toMutableSet() ?: mutableSetOf("HUL")
    ...
    override fun onDestroy() {
        ...
        wifiDirectDetectDialog = null
        wifiDirectNameDiscoveryManager?.stop()
        wifiDirectNameDiscoveryManager = null
        super.onDestroy()
    }
            ...
            when (data.host) {
                "start" -> {
                    ...
                    if (!modeParam.isNullOrEmpty()) {
                        ...
                        if (modeIdx != -1) {
                            val normalizedMode = normalizeConnectionMode(modeIdx)
                            getSharedPreferences("WirelessHelperPrefs", Context.MODE_PRIVATE).edit { putInt("connection_mode", normalizedMode) }
                            tvConnectionModeValue.text = connectionModeOptions.firstOrNull { it.mode == normalizedMode }?.label
                                ?: connectionModeOptions.first().label
                            updateModeSpecificUI(normalizedMode)
                        }
                    }
    ...
    private fun handleConfigIntent(uri: android.net.Uri) {
        ...
        if (ssid != null) {
            ...
            if (pass != null) {
                org.xs.hulhelper.utils.Prefs.getSecure(this).edit {
